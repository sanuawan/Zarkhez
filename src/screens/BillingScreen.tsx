...
const BillingScreen = () => {
  ...
  // ✅ NEW: Filter state (By default pending show ho ga)
  ...
  useEffect(() => {
    const unsubRate = firestore().collection('settings').doc('billing_config').onSnapshot(doc => {
      if (doc.exists) setGlobalRate(doc.data()?.currentRate.toString() || '200');
    });

    const unsubHistory = firestore().collection('billing_history').onSnapshot(snap => {
      const grouped: any = {};
      snap.forEach(doc => {
        const data = doc.data();
        const name = data.userName || 'Unknown User';
        const status = data.status || 'pending'; // ✅ Auto-detect status from Firebase
        ...
        if (!grouped[name]) {
          grouped[name] = { 
            ...
            totalBill: 0, 
            sessionCount: 0,
            status: status // ✅ Store status for UI
          };
        }
    ...
    return () => { unsubRate(); unsubHistory(); };
  }, [filterStatus]); // ✅ Added filterStatus here to trigger update

  const handleUpdateRate = () => {
    const rateNum = parseFloat(globalRate);
    if (isNaN(rateNum)) return Alert.alert("Error", "Invalid Rate");
    firestore().collection('settings').doc('billing_config').update({ currentRate: rateNum })
      .then(() => { setIsEditingRate(false); Alert.alert("Success", "Rate Updated!"); });
  };
    ...
    <SafeAreaView style={styles.container}>
      ...
      <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={styles.scrollContent}>
        ...
        <View style={styles.content}>
          ...
          {/* ✅ NEW: FILTER TABS - Designing achi kr di h */}
          ...
          {usersData.map((user) => (
            <View key={user.id} style={styles.userRowContainer}>
              ...
              {/* ✅ NEW: STATUS LABEL - Top right corner mian small h */}
              <View style={{ position: 'absolute', right: 12, top: 10, paddingHorizontal: 8, paddingVertical: 2, borderRadius: 4, backgroundColor: user.status === 'paid' ? '#E8F5E9' : '#F5F5F5', zIindex: 5 }}>
              ...
              <TouchableOpacity style={styles.detailsButton} onPress={() => navigation.navigate('UserDetail' as never, { userName: user.name } as any)}>
